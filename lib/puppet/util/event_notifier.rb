...
# event_notifier.rb
module Puppet::Util::Event_notifier
  def settings
    return @settings if @settings
    @settings_file = Puppet[:confdir] + '/event_notifier/settings.yaml'
  ...
  def secrets
    ...
    @secrets_file = Puppet[:confdir] + '/event_notifier/secrets.yaml'
    ...
    if http.use_ssl?
      if (settings['ssl_ca'] && !settings['ssl_ca'].empty?) || settings['include_system_cert_store']
        ssl_ca_file = if settings['ssl_ca']
                        File.join(Puppet[:confdir], 'event_notifier', settings['ssl_ca'])
